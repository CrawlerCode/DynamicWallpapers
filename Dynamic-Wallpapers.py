import win32con, os, win32gui, time, psutil, PySimpleGUIQt, Modules
...
cfg = config.Config(BASE_PATH, default_config={"Wallpaper": None, "Modules": [], "Font": "segoeuil.ttf"})
...
class Menu:
    ...
    def _get_menu(self):
        self.wallpapers = ['Refresh Wallpapers', 'Open Wallpaper Folder', '---']
        ...
        for module in self.wallpaper.modules:
            ...
            if len(settings) > 0:
                self.modules.append(settings)
        menu = ['menu', ['Wallpapers', self.wallpapers, 'Modules', self.modules, 'Settings', ['Fonts', self.fonts], '---', 'Exit Dynamic-Wallpapers']]
        return menu
...
class Wallpaper:
    ...
    def __init__(self):
        ...
        self.fonts = {}
        self.updateFont()
        self.modules = Modules.getAllModules(self, cfg, cfgData)
        self.menu = None

    def updateImage(self):
        try:
            self.img = Image.open(PICTURE_PATH + cfgData["Wallpaper"]).convert("RGBA")
        except:
    ...
    def getFont(self, size):
        if size in self.fonts:
            ...
            self.fonts[size] = ImageFont.truetype('/Library/Fonts/' + self.font, size)
            return self.fonts[size]

    def getFontHeight(self, size):
        return self.draw.textsize(text="text", font=self.getFont(size))[1]

    def getFontWidth(self, text, size):
        return self.draw.textsize(text=text, font=self.getFont(size))[0]
                    ...
                    try:
                        ...
                        for module in self.modules:
                            if module.active is True:
                                height += module.handle(img, draw, height)
                        img.save(self.path)
